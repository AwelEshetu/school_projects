...
const unsigned int uppermargin =70;
unsigned int x;
unsigned int y;
...
-(void)drawPrevButton:(CGPoint)leftTop{
    ...
    CGContextMoveToPoint(ctx,leftTop.x , 10+leftTop.y);
...
-(void)drawNameOfDaysAtTop {
    ...
    CGPoint location = CGPointMake(width/2-uppermargin, prevNextButtonSpaceHeight); //location holds the points set by cgpointmake
	...
	[[UIColor colorWithRed:2.0 green:2.0 blue:2 alpha:0.9]set];
...
-(void) touchAtDate:(CGPoint)touchPoint{
    ...
    if(monthday>0 && monthday<monthDayCount+1)
    {
        ...
        if( [delegate1 respondsToSelector:@selector(getDate:)])
       	...
       	UILabel *myLabel = [[UILabel alloc] initWithFrame:CGRectMake(108, 350, 90, 20)]; 
        tempstring=[NSString stringWithFormat:@"%ld-%ld-%d",currentSelectDate.year,currentSelectDate.month,monthday];
        myLabel.text=tempstring;
        myLabel.backgroundColor = [UIColor grayColor];
        
        [self.window addSubview:myLabel];
        // NSLog(@"year=%@  month=%@  day=%d",currentSelectDate.year,currentSelectDate.month,monthday);
        NSLog(@"%@",tempstring);
    }
    else{
        UILabel *myLabel = [[UILabel alloc] initWithFrame:CGRectMake(108, 350, 90, 20)]; 
...
- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if(touchPoint.x<40 && touchPoint.y<60)
		[self movePrevMonth];
	else if(touchPoint.x>width-40 && touchPoint.y<60)
		[self moveNextMonth];
...
-(void) drawToday{
    ...
    if(today.month == currentMonthDate.month && today.year==currentMonthDate.year){
		...
		CGContextMoveToPoint(ctx, x*s_width+40, y*35+70);
		CGContextAddLineToPoint(ctx, x*s_width+s_width+40, y*35+70);
		CGContextAddLineToPoint(ctx, x*s_width+s_width+40, y*35+105);
		CGContextAddLineToPoint(ctx, x*s_width+40, y*35+105);
		CGContextFillPath(ctx); 
        
        
        
        CGContextSetRGBFillColor(ctx, 2, 2, 2, 1);
        UIFont *weekfont = [UIFont boldSystemFontOfSize:15];
        NSString *date = [[NSString alloc]initWithFormat:@"%2d",today.day];
        
        [date drawAtPoint:CGPointMake(x*s_width+50, y*35+80) withFont:weekfont];
